...
###########################################################################
def k_truss_to_l(graph: CSRCOO, n_cut, num_v, l_max, support):  #从l往上分解的算法设计
    ...
    return e_truss, ptr_truss, t11, t22
#############################################################################
def k_truss_from_l(graph: CSRCOO, n_cut, l_max, support):  
    torch.cuda.synchronize()
    t11 = time.time()
    #计算支持值
    # support = support_computing(graph.rows, graph.columns, graph.row_ptr, n_cut)
    ...
    if (mask.shape[0] - graph.columns.shape[0]) > 100000000:
        print("new number:")
        values = values.view(-1, n_cut)
        e_mask = torch.sum(values.bool(), dim=1)
        e_mask[torch.unique(graph.columns)] = True
        s_index = torch.nonzero(e_mask).squeeze(1)  #这下面的顶点重新编号，还涉及分块要考虑  #为什么分块数为1时，计算结果还是错的呢？忽略了没有更大邻居的columns
        num_v = s_index.shape[0]
        graph.row_ptr = torch.flatten(values[s_index])
        graph.row_ptr = torch.cat([torch.zeros(1, dtype=torch.int32, device=graph.device), graph.row_ptr.cumsum(0).to(torch.int32)]) #所以row_ptr的开头是[0, 0]
        values = values[:, 0] 
        values[s_index] = torch.arange(0, s_index.shape[0], dtype=torch.int32, device=graph.device) #从1开始编号的
        graph.rows = values[graph.rows]
        graph.columns = values[graph.columns]
    else:
        num_v = graph.row_ptr.shape[0]
    # num_v = graph.row_ptr.shape[0]
    #重新计算支持度
    support = support_computing(graph.rows, graph.columns, graph.row_ptr, n_cut)
    #找到要删除的边
...
def k_truss_from_l_to_l(graph: CSRCOO, n_cut, l_min, l_max, support, sdevice):  
    # print("l_max:", l_max)
    torch.cuda.synchronize()
    t11 = time.time()
    #计算支持值
    ...
    #计算边映射序号             
    l = l_min
    edges = torch.arange(graph.columns.shape[0], device=sdevice)
    #第一步，整理整个图，支持度为零的数据清除
    mask = support >= l
    # support = support[mask]
    graph.columns = graph.columns[mask]
    graph.rows = graph.rows[mask]
    edges = edges[mask]
    values = torch.zeros(graph.row_ptr.shape[0]-1, dtype=torch.int32, device=sdevice)
    segment_add(mask.int().to(torch.int32), graph.row_ptr, values)
    graph.row_ptr = torch.cat([torch.zeros(1, dtype=torch.int32, device=sdevice), values.cumsum(0).to(torch.int32)])
    #顶点重编号
    values = values.view(-1, n_cut)
    e_mask = torch.sum(values.bool(), dim=1)
    e_mask[torch.unique(graph.columns)] = True
    ...
    num_v = s_index.shape[0]
    graph.row_ptr = torch.flatten(values[s_index])
    graph.row_ptr = torch.cat([torch.zeros(1, dtype=torch.int32, device=sdevice), graph.row_ptr.cumsum(0).to(torch.int32)]) #所以row_ptr的开头是[0, 0]
    values = values[:, 0] 
    ...
    support = support_computing(graph.rows, graph.columns, graph.row_ptr, n_cut)
    #找到要删除的边
    e_mask = torch.zeros(graph.columns.shape[0], dtype=torch.bool, device=sdevice)
    e_curr = torch.where(support<l)[0]
    #将不属于 l truss 的边都标记并删除
    while e_curr.shape[0] != 0:
        p = torch.unique(graph.rows[e_curr]) #这里面就不该有-1
        ...
        e_affect = torch.nonzero(mask).squeeze(1).to(torch.int32) 
        e_mask[e_curr] = True #看看能不能改代码，将e_mask标记成剩余边
        n_mark = torch.zeros(graph.columns.shape[0], dtype=torch.bool, device=sdevice)
        all_affect_support(e_affect, graph, n_cut, e_mask, l, n_mark, support)
        graph.columns[e_curr] = -1 
        # mask = (support<l) & (~e_mask)
        # e_curr = torch.where(mask)[0] 
    ...
    while True:
        ...
        while e_curr.shape[0] == 0:
            ...
            if l == l_max:
                ...
                print('From l to l truss decomposition Completed! {}s time elapsed. Outputting results...'.format(t22 - t11))
                return e_truss, ptr_truss, t11, t22
            e_curr = torch.where(support == l)[0] 
        # if e_curr.shape[0] == 0:
        #     # print("before l:", l)
        ...
        #         break
        #     e_curr = torch.where(support == l)[0]  #也许这里
        #     while e_curr.shape[0] == 0 and l < l_max:
        #         ptr_truss = torch.cat([ptr_truss, ptr_truss[-1].unsqueeze(0)])
        #         l += 1
        #         e_curr = torch.where(support == l)[0] 
    torch.cuda.synchronize()
...
def find_split_l(graph, n_cut, num_hpus): #1. 指数探索+二分查找 找到l; 2. 要分割数据吗？  #或者通过k_core_max来分割
    ...
    l = 1
    half = graph.columns.shape[0]//(num_hpus+33)  
    support = support_computing(graph.rows, graph.columns, graph.row_ptr, n_cut)
    temp_count = 0
...
def find_split_l2(graph, n_cut, num_hpus): #1. 指数探索+二分查找 找到l; 2. 要分割数据吗？  #或者通过k_core_max来分割
    ...
    half = torch.sum(support)//(num_hpus+300) 
    temp_count = 0
    for i in range(num_hpus-1):
        while temp_count < half:
            l = l*2
...
def gpu_vs_cpu(graph, n_cut):
    ...
    t22 = time.time()
    t_cpu = t22 - t11
    print('cpu support {}s time elapsed. Outputting results...'.format(t_cpu))
    torch.cuda.synchronize()
    graph.to('cuda')
    t11 = time.time()
    support = support_computing(graph.rows, graph.columns, graph.row_ptr, n_cut) 
    ...
    t22 = time.time()
    t_gpu = t22 - t11
    print('gpu support {}s time elapsed. Outputting results...'.format(t_gpu))
    print('gpu vs cpu : {}'.format(t_cpu//t_gpu))
...
class mutilGPU_ktruss(Process):
    def __init__(self, rank, size, graph, n_cut, num_v, l_split, support, **kwargs):
        ...
        if self.rank == 0:
            ...
            self.device = torch.device(f'cuda:{int(self.rank-1)}')
        self.graph = graph
        self.n_cut = n_cut
        self.num_v = num_v
    ...
    def run(self):
        ...
        if self.rank == 0:
            ...
            k_truss_to_l(self.graph, self.n_cut, self.num_v, self.l_split[gpu_id].item(), self.support)
        elif self.rank == (self.size-1):
            print(f"Rank: {self.rank}, World Size: {dist.get_world_size()}")
            torch.cuda.set_device(self.device)
            # 消除初始化环境的影响
            self.graph.row_ptr = self.graph.row_ptr.to(self.device)
            self.graph.columns = self.graph.columns.to(self.device)
            self.graph.rows =  self.graph.rows.to(self.device)
            self.support = self.support.pin_memory()
            self.support = self.support.to(self.device)
            print("suppport device:", self.support.device)
            k_truss_from_l(self.graph, self.n_cut, self.l_split[-1].item(), self.support)
        else:
            print(f"Rank: {self.rank}, World Size: {dist.get_world_size()}")
            torch.cuda.set_device(self.device)
            # 消除初始化环境的影响
            ...
            self.support = self.support.pin_memory()
            self.support = self.support.to(self.device)
            k_truss_from_l_to_l(self.graph, self.n_cut, self.l_split[gpu_id-1].item(), self.l_split[gpu_id].item(), self.support, self.device)
...
def main_csrcgraph(args):
    ...
    # gpu_vs_cpu(graph, n_cut)
    l_split, support = find_split_l(graph, n_cut, num_hpus)
    ...
    k_truss_from_l(graph, n_cut, 5, support)
    ...
    #     p = mutilGPU_ktruss(rank, num_hpus, graph, n_cut, num_v, l_split, support)  #rank, size, graph,
    #     p.start()
    #     processes.append(p)
