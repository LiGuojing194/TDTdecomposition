...
def truss_deposs(graph):
    ...
    #     mask = torch.isin(graph.columns, non_leaf) & torch.isin(row_indice_g, non_leaf)
    #     graph.columns = graph.columns[mask]
    #     row_indice_g = row_indice_g[mask]
    #     sizes = segment_csr(mask.int(), graph.row_ptr, reduce='sum')
    #     graph.row_ptr = torch.cat([torch.zeros(1, dtype=torch.int64, device=graph.row_ptr.device), sizes.cumsum(0)])
    # del non_leaf
    #获取一份无向图数据
    ...
    # print("row_indice type:", row_indice.dtype)
    # print('columns type', columns.dtype)
    # print('direc_row_ptr type', direc_row_ptr.dtype)
    # print('support type', support.dtype)
    print("row_indice:", row_indice)
    print("columns:", columns)
    print("direc_row_ptr:", direc_row_ptr)
    ...
    while True:
        ...
        if left_nbr.shape[0]==0 or right_nbr.shape[0] == 0:
            ...
            # e_curr_rep = torch.repeat_interleave(e_curr, e_ptr[1:] - e_ptr[:-1])[mask2]
            # temp = segment_csr(mask1.int(), s_ptr, reduce="sum")
            ...
            # sub_count = torch.zeros(support.shape[0], dtype=torch.int64, device=support.device) 
            # ######批量要对三种情况进行处理，会更慢吧#############
            # #计数减一  用一个cuda函数来使得不用三种情况轮流处理一遍
            # mask_temp = mask1 & mask2
            # unique_e, counts = torch.unique(left_nbr[mask_temp], return_counts=True)
            # sub_count[unique_e] = sub_count[unique_e] + counts
            # unique_e, counts = torch.unique(right_nbr[mask_temp], return_counts=True)
            # sub_count[unique_e] = sub_count[unique_e] + counts
            # #处理mask1&~mask2   8ms（建立索引更新索引较慢，要扫描所有元素）
            # mask3 = mask1&(~mask2)
            # mask4 = e_curr_rep[mask3]<right_nbr[mask3] 
            # unique_e, counts = torch.unique(left_nbr[mask3][mask4], return_counts=True)
            # sub_count[unique_e] = sub_count[unique_e] + counts
            # #处理mask2&~mask1
            # mask3 = (~mask1)&mask2
            # mask4 = e_curr_rep[mask3]<left_nbr[mask3] #当前边号小，需要减一  
            ...
            # print("left_nbr:", left_nbr)
            # print("right_nbr:", right_nbr)
            # print("e_curr_rep:", e_curr_rep)
            # print("support:", support)
