...
def k_truss(graph: CSRCOO, n_cut, num_v):  
    ...
    graph.row_ptr = torch.cat([torch.zeros(1, dtype=torch.int32, device=graph.device), size_r.cumsum(0, dtype=torch.int32)])
    #################################### 
    ...
    while True:
        ...
        if e_peeling_count > 10000000:
            ...
            #更新graph.row_ptr
            graph.row_ptr = update_row_ptr(e_mask, graph.row_ptr)
            if (col_count/graph.columns.shape[0])>3 and n_cut < 64:
