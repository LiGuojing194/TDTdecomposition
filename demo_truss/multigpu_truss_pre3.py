...
def k_truss(graph: CSRCOO, n_cut, num_v):  
    ...
    while True:
        ...
        if e_peeling_count > 10000000:
            ...
            graph.rows = graph.rows[e_mask]
            edges = edges[e_mask] 
            # if (col_count/graph.columns.shape[0])>4 and n_cut < 32:
...
def csr_to_tilingcsr(graph: CSRCOO, tiling, n_cut):
    ...
    split_t = torch.zeros(n_cut+1, dtype=torch.int32, device=graph.device)
    # for i in range(1, n_cut):
    #     split_t[i] = split_t[i-1] + v_num//(2**i)
    split_t[1] = v_num // (2**0.5)
