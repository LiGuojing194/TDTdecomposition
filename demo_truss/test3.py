...
def k_truss(graph: CSRCOO, n_cut, num_v):  
    ...
    # graph.columns = graph.columns[:, e_mask_c]
    ...
    while True:
        ...
        if (e_truss.shape[0] + e_curr.shape[0]) == support.shape[0]:  #如何正确跳出循环
            ...
            ptr_truss = torch.cat([ptr_truss, torch.tensor([e_c_truss.shape[0]], dtype=torch.int32, device='cpu')])
            break
        if e_curr.shape[0] == 0:
            if e_truss.shape[0] > 10000000:
                ...
                ptr_truss = torch.cat([ptr_truss, torch.tensor([e_c_truss.shape[0]+e_truss.shape[0]], dtype=torch.int32, device='cpu')])
