...
###########################################################################
def find_split_l(graph, n_cut): #1. 指数探索+二分查找 找到l; 2. 要分割数据吗？  #或者通过k_core_max来分割
    l = 1
    half = graph.columns.shape[0]//3*2   #//3*2
    ...
    # l = 24
    ###################
    # l += 6*int((torch.log(torch.max(support)-l) / torch.log(torch.tensor(2))).item())
    # l = 2*l+8
    # l = int((l + half/graph.row_ptr.shape[0]))
    l = 8
    print("The found l is :", l)
    return l, support
...
def k_truss_to_l(graph: CSRCOO, n_cut, num_v, l_max, support):  #从l往上分解的算法设计
    ...
    return e_truss, ptr_truss, t11, t22
#############################################################################
def k_truss_from_l(graph: CSRCOO, n_cut, num_v, l_max, support):  
    torch.cuda.synchronize()
    t11 = time.time()
    #计算支持值
    # support = support_computing(graph.rows, graph.columns, graph.row_ptr, n_cut)
    ...
    graph.row_ptr = torch.cat([torch.zeros(1, dtype=torch.int32, device=graph.device), graph.row_ptr.cumsum(0, dtype=torch.int32)]) #所以row_ptr的开头是[0, 0]
    values = values[:, 0] 
...
def main_csrcgraph(args):
    ...
    print("graph.ptr shape:", graph.row_ptr.shape[0], graph.row_ptr.dtype)
    n_cut = 2
    ...
    # l_max, support = find_split_l(graph, n_cut)
    # k_truss_to_l(graph, n_cut, num_v, l_max, support)
    # k_truss_from_l(graph, n_cut, num_v, l_max, support)
